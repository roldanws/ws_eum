...
def solicitarCambio(cambioSolicitado):
	...
	for moneda in MONEDAS:
		...
		if(cambio):
			...
			cambioHopper(cambio,HOPPER_MONEDAS_HABILITADAS[moneda])
			print("monedas solicitadas de ",moneda,": ",cambio)
			time.sleep(1)
...
def resetHopper(hopperId):
	...
	ser2.write(a);
	r = ser2.read(20) #Verificar en el simulador se ven 19
	print("resetHopper ", lenresetHopper, "HOPPER: ", hopperId)
	r = r[lenresetHopper:]
	time.sleep(.01)
...
def cambioHopper(cambioSolicitado, hopperId):
	...
	ser2.write(dispenseHopper);
	time.sleep(.01)
	r = ser2.read(20) #Verificar en el simulador se ven 19
	#print("longitud comando ", lendispenseHopper)
	r = r[lendispenseHopper:]
	time.sleep(.01)
	validarCctalk(r,HOPPER_DISPENSE,hopperId,VALIDACION_EXACTA)
	return 1
...
def darCambio(ser,monto):
	while(1):
		...
		ser.write(b'\x02')
		ser.parity = change_parity(int(dar), 0)u
...
if __name__ == "__main__":
	...
	"""
	ser2 = PuertoSerie ("PuertoSerie")
	ser2.modificarConfiguracion (dispositivo = PuertoSerie.PUERTO_ARDUINO_MICRO, baudrate = 9600)
	ser2.start()
	ser2.abrirPuerto()
	"""
	...
	ser = serial.Serial("/dev/ttyACM1")  # Open named port
	ser2 = serial.Serial("/dev/ttyACM0")  # Open named port
	...
	ser2.baudrate = 9600  # Set baud rate
