...
class GuardarLogs ():
    def __init__(self, nombreDelArchivo = None, directorio = None):
        ...
        if directorio is not None:
            self.ruta = directorio
            print ("Imprimiendo la ruta para almacenar logs", ruta)
        else:
            self.ruta = ruta
    ...
    def abrir(self, archivoActual = 0):
        ...
        aux_0 = 1 
        print("imprimiendo ruta ", self.ruta)
        while(aux_0):
            try:
                if not os.path.exists(self.ruta):
                    os.mkdir(self.ruta)
                    #print("Directorio " , self.ruta ,  " Creado ")
                else:    
                    #print("Directorio " , self.ruta ,  " ya exite")
                    pass


                self.archivo = open (self.ruta + self.nombreDelArchivo + "_" + str(self.numeroConsecutivo), "r", encoding='ISO-8859-1')
                aux = True
                #print ("El archivo existe", self.archivo)
                self.archivo.close()
        ...
        self.archivo = open (self.ruta + self.nombreDelArchivo + "_" + str(self.numeroConsecutivo), "w", encoding='ISO-8859-1')
    ...
    def print (self, *args, **kwargs):
        ...
        for i, arg in enumerate(args):
            ...
            if len(args) > 0 and arg!="\n" and i <len(args)-1:
                texto += " "

        self.actualizarDia()


        self.archivo = open (self.ruta + self.nombreDelArchivo + "_" + str(self.numeroConsecutivo), "a", encoding='ISO-8859-1')
